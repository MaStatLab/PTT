...
double GBT::make_rho0(int level) {
    ...
    if (rho_mode == 1) { // 1 - rho0 decreases with level quadratically
      return 1-(1-rho0)/(level+1)/(level+1);
    }
    if (rho_mode == 2) { // 1 - rho0 decreases with level exponentially
...
vector<double> GBT::compute_predictive_density(Mat< unsigned int >Xnew) {
  ...
  for (int i=0; i < nobs_new; i++) { //since we do it only once, do this for now
    ...
    update_subtree_add_new_data(I_root,0,1,0,Xnew.row(i).t());
    new_root_logphi = get_root_logphi();

    predictive_density[i] = exp(new_root_logphi - original_root_logphi);
    update_subtree_remove_new_data(I_root,0,1,0,Xnew.row(i).t());
  }
